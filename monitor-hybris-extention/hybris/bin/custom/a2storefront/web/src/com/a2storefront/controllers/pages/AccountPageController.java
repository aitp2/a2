 ...
 */
package com.a2storefront.controllers.pages;
...
import com.a2.a2facade.order.A2ConfirmReceipt;
import com.a2storefront.controllers.ControllerConstants;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	@Resource(name = "a2defaultConfirmReceipt")
	private A2ConfirmReceipt a2defaultConfirmReceipt;
			...
			throws CMSItemNotFoundException
	{
		...
		//searchPageData.getResults().get(0).getStatus().getCode()
	 ...
	 * @throws CMSItemNotFoundException
	 */
	@RequestMapping(value = "/confirmReceipt/" + ORDER_CODE_PATH_VARIABLE_PATTERN, method = RequestMethod.GET)
	@RequireHardLogIn
	public String confirmReceipt(@PathVariable("orderCode") final String orderCode, final Model model,
			final RedirectAttributes redirectModel,@RequestParam(value = "page", defaultValue = "0") final int page,
			@RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode,
			@RequestParam(value = "sort", required = false) final String sortCode) throws CMSItemNotFoundException
	{
			// Handle paged search results
		if(StringUtils.isNotEmpty(orderCode)){
			a2defaultConfirmReceipt.confirmReceipt(orderCode);
		}
		final PageableData pageableData = createPageableData(page, 5, sortCode, showMode);
		final SearchPageData<OrderHistoryData> searchPageData = orderFacade.getPagedOrderHistoryForStatuses(pageableData);
		populateModel(model, searchPageData, showMode);
